...
def skeleton_to_nx(skeleton: Skeleton, summary: pd.DataFrame | None = None):
    ...
    for row in summary_future.itertuples(name='Edge'):
        ...
        # print(f"#### Summary data stored in edges...")
        # print(f"{g.edges[i, j]=}\n")
        g.edges[i, j]['path'] = skeleton.path_coordinates(row.Index)
        # print(f"~~~~ Additional path co-ordinates added...")
        # print(f"{g.edges[i, j]['path']}\n####\n")
...
def nx_to_skeleton(g: nx.Graph, orig_dim: tuple) -> Skeleton:
    ...
    which stores the summary information of the Skeleton as attributes to the
    edges and allows reconstruction.
    ...
    Skeleton
        Skeleton object.
    """
    summary = _reconstruct_summary_from_graph_attributes(g)
    np_img = np.zeros(orig_dim)
    start_coords = summary[["coord_src_0", "coord_src_1"]].to_numpy()
    end_coords = summary[["coord_dst_0", "coord_dst_1"]].to_numpy()
    skeleton_coordinates = None
    skeleton_coordinates = _bresenhams_line(start_coords[0], end_coords[0])
    for start, end in zip(start_coords[1:], end_coords[1:]):
        coordinates = _bresenhams_line(start, end)
        skeleton_coordinates = np.append(
                skeleton_coordinates, coordinates, axis=0
                )
    image = np.zeros(orig_dim)
    # Replace all co-ordinates of skeleton with 1
    image[tuple(skeleton_coordinates.T)] = 1
    return Skeleton(image)
    # ToDo...
    #
    # 1. Find the maximum coordinate of coord_src_0, coord_src_1 and coord_dst_0, coord_dst_1
    # 2. Create an empty array of zeros of this dimension
    # 3. Itereate over coord_src_0, coord_src_1 and coord_dst_0, coord_dst_1 and fill in points between with 1


def _reconstruct_summary_from_graph_attributes(g: nx.Graph) -> pd.DataFrame:
    """Reconstruct a skeletons summary from the attributes saved in the edges
    of NetworkX Graph.

    Parameters
    ----------
    g: nx.Graph
        Network X Graph which was generated by skeleton_to_nx() and stores the
    summary information in the edges.

    Returns
    -------
    pd.DataFrame
        Pandas dataframe of skeletons summary information.
    """
    summary = pd.DataFrame()
    for edge in g.edges():
        i, j = edge
        attributes = g.edges[i, j]
        attributes.pop("path")
        summary = pd.concat([
                summary,
                pd.DataFrame.from_dict([attributes], orient="columns")
                ])
